// ═══════════════════════════════════════════════════════════════════════════
// QOMN v3.0 — LLVM IR Backend
// Emits LLVM IR 18 text from QOMN oracle definitions.
// The IR can be compiled to native code via:
//   llc-18 -O3 -filetype=obj out.ll -o out.o
//   clang-18 -shared -fPIC out.o -o libqomno.so
// ═══════════════════════════════════════════════════════════════════════════
...
pub fn oracle_to_llvm_ir(oracle: &OracleDecl) -> String {
    ...
    // Module header
    out.push(format!("; QOMN v3.0 LLVM IR — oracle: {}", oracle.name));
    out.push("; Generated by qomn llvm_backend".into());
    out.push("; Compile: llc-18 -O3 -filetype=obj out.ll -o out.o".into());
    out.push(";          clang-18 -shared -fPIC out.o -o libqomn_oracle.so".into());
    out.push("".into());
    out.push("source_filename = \"qomn_oracle\"".into());
